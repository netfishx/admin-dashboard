    ...
    <div className="flex flex-col rounded-sm bg-card p-4">
      <div className="flex items-center justify-around pb-2">
        <Item
          ...
          value={formatNumber(Number(data?.baccaratShareAmount ?? 0))}
          icon={welcome01}
        />
        <Item
          name={t("backAmount")}
          value={formatNumber(Number(data?.backAmount ?? 0))}
          icon={welcome02}
        />
        <Item
          name={t("supplierCost")}
          value={formatNumber(Number(data?.supplierCost ?? 0))}
          icon={welcome03}
        />
        <Item
          name={t("grossProfitLoss")}
          value={formatNumber(Number(data?.grossProfitLoss ?? 0))}
          icon={welcome04}
          unShowBorder
        />
      </div>
